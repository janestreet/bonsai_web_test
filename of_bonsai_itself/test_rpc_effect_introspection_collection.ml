...
let%test_module "More interesting polling_state_rpc diffs" =
  (module struct
    module Polling_state_rpc = struct
      module Response = struct
        type t =
      ...
      let polling_state_rpc_implementation =
        let m name res =
          name, { Response.name; capybaras = String.Map.of_alist_exn res }
        in
...
let%test_module "Rpc_effect.Rpc.poll_until_condition_met and \
